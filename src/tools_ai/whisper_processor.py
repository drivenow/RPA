...
def process_audio(wav_file, model_name="base.en"):
    ...
    model = f"X:/github/whisper.cpp/models/ggml-{model_name}.bin"
    ...
    full_command = f"X:/github/whisper.cpp/build/bin/Release/whisper-cli.exe -m {model} -f {wav_file} -nt"
    print(full_command)

    # Execute the command
    process = subprocess.Popen(full_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, 
    encoding='utf-8',  # 新增显式编码声明
    ...
    decoded_str = output#.decode(output_encoding).strip()
